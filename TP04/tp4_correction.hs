...
-- On reprend le type arbre binaire non etiquete du CM #05
--
...
-- 0.1 Petit utilitaire de visualisation dÕun (Tree a)
--
...
visuTree t = visu t 1
   where visu (Tip x) n = [' ' | i <- [1..n]] ++ (show x) ++ ['\n']
         visu (Bin t1 t2) n = (visu t1 (n+5)) ++ 
                              ...
                              (visu t2 (n+5))
{- 
Exemples dÕutilisation sur des textes de petite taille
...
-- 0.2 Un exemple dÕarbre de plus grande taille (bati sur alphabet)
--
...
-- DŽterminer le codage du caractre x selon t
...
-- codeString t = concat . map (codeChar t)

code :: Tree Char -> String -> Chemin
code t xs = concat (map (codeChar t) xs)
...
--
-- Soit donner le code et leur demander ce que cela fait, 
--   en particulier le t supplementaire qui n'evolue pas ?
-- Soit trouver une autre formulation en rendant global l'arbre ?
...
ajouteFrequence c ((c2,n):cs) 
   | c == c2   = (c2,n+1):cs
   | otherwise = (c2,n):(ajouteFrequence c cs)

qsort :: Ord b => [(a,b)] -> [(a,b)]
qsort [] = []
qsort (x:xs) = qsort [ y | y <- xs, snd y <= snd x ] 
               ++ [x]
               ++ qsort [ y | y <- xs, snd y > snd x ]
...
codeText xs = map v (code (build (calculFrequence xs)) xs)
   where v Gauche = '0'
         v Droite = '1'
...
tauxDeCompression xs = 1.0 - (fromIntegral (length (code (build (calculFrequence xs)) xs))) / (fromIntegral (8 * (length xs)))
